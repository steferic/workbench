...
pub fn handle_parallel_action(
    ...
    match action {
        Action::StartParallelTask => {
            start_parallel_task(state, pty_manager, action_tx)?;
        }
        Action::CancelParallelTask(task_id) => {
            cancel_parallel_task(state, task_id, pty_manager)?;
        }
        Action::SelectParallelWinner(attempt_id) => {
            select_parallel_winner(state, attempt_id, pty_manager)?;
        }
        ...
        Action::MergeSelectedReport => {
            merge_selected_report(state, pty_manager)?;
        }
        _ => {}
    }
    Ok(())
...
fn start_parallel_task(
    ...
    // Check if this is a git repo (worktrees will handle any dirty state)
    ...
    // Clean up any existing active parallel tasks before creating a new one
    ...
    // Create worktrees and sessions for each agent
    for agent_type in selected_agents {
        ...
        match pty_manager.spawn_session(
            ...
            Ok(handle) => {
                ...
                // Initialize last_activity so the session isn't immediately considered "idle"
                // This prevents the prompt from being sent before the agent CLI has started
                state.data.last_activity.insert(session_id, std::time::Instant::now());
                // Prompt will be sent when the session becomes idle (via handler.rs)
            }
            Err(_) => {
...
fn select_parallel_winner(
    ...
    let (workspace_path, workspace_id, task_id, source_branch, winner_branch, session_ids, worktree_paths) = {
        ...
        (ws.path.clone(), ws.id, task.id, task.source_branch.clone(), attempt.branch_name.clone(), ids, paths)
    };

    // Merge the winning branch
    git::checkout_branch(&workspace_path, &source_branch)?;
    git::merge_branch(&workspace_path, &winner_branch)?;
    ...
    if let Some(ws) = state.data.workspaces.get_mut(state.ui.selected_workspace_idx) {
        if let Some(task) = ws.get_parallel_task_mut(task_id) {
            task.mark_completed(attempt_id);
        }
        ws.remove_parallel_task(task_id);
    }
...
fn merge_selected_report(state: &mut AppState, pty_manager: &PtyManager) -> Result<()> {
