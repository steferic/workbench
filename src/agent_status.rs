...
/// The command an agent should run for each hook event.
///
/// Prefers `wbhook` sitting beside the workbench binary — a 350 KB forwarder
/// with no dependencies, against 7.4 MB of TUI, runtime, SQLite and audio that
/// a hook has no use for. At ~1,450 invocations a minute that is the
/// difference between 1.6 MB resident per event and 9.7 MB, which is what the
/// kernel was objecting to every time it loaded the big one.
///
/// Falls back to `workbench hook`, which still does the same job, so an
/// install that predates the forwarder — or one where only the main binary was
/// copied somewhere — keeps reporting rather than going quiet. Status drives
/// idle detection and the TODO queue; losing it silently would be worse than
/// the cost it saves.
fn hook_command(workbench_bin: &str) -> String {
    let forwarder = Path::new(workbench_bin).with_file_name("wbhook");
    if forwarder.is_file() {
        return shell_quote(&forwarder.to_string_lossy());
    }
    format!("{} hook", shell_quote(workbench_bin))
}
...
pub fn claude_hook_settings(workbench_bin: &str) -> String {
    let hooks: serde_json::Map<String, serde_json::Value> = CLAUDE_HOOK_EVENTS
        .iter()
        .map(|event| {
            let command = format!("{} {event}", hook_command(workbench_bin));
            (
                (*event).to_string(),
                serde_json::json!([{
                    "matcher": "",
